...
func (e *Engine) SendToSession(sessionKey, message string) error {
	return e.SendToSessionWithMetadata(sessionKey, message, nil, nil, nil, nil, false)
}
...
func (e *Engine) SendToSessionWithMetadata(sessionKey, message string, images []ImageAttachment, files []FileAttachment, metadata map[string]string, atUsers []string, atAll bool) error {
	return e.SendToSessionWithOptions(sessionKey, message, images, files, SendOptions{Metadata: metadata, AtUsers: atUsers, AtAll: atAll})
}
...
func (e *Engine) SendToSessionWithOptions(sessionKey, message string, images []ImageAttachment, files []FileAttachment, opts SendOptions) error {
	...
	if len(opts.Metadata) > 0 {
		if storer, ok := p.(ProactiveContextStorer); ok {
			storer.StoreProactiveContext(sessionKey, opts.Metadata)
		}
	}
